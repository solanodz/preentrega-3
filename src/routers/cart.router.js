...
router.get('/carts', async (req, res) => {
    let result = await CartController.getCart()
...
router.get('/carts/:cid', async (req, res) => {
    ...
    const cart = await CartController.getCart(cid, true);
    if (!cart) {
...
router.post('/carts', async (req, res) => {
    let result = await CartController.addNewCart({})
    res.status(201).json(result)
})
...
router.delete('/carts/:cid/products:/:pid', async (req, res) => {
    ...
    try {
        await CartController.deleteFromCart(cid, pid);
...
router.post('/carts/:cid/products/:pid', async (req, res) => {
    ...
    try {
        await CartController.addProduct(cid, pid, quantity);
